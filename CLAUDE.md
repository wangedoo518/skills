...
| **Aurora plugin**（视频 / 图文解析，见 ADR-022）| `~/.hermes/plugins/xiaohongshu/` ← junction → `./aurora/plugins/xiaohongshu/` | ✅ 老板 wangedoo518/aurora 集成完成；提供 `xhs_extract_note(url, ocr, transcribe, extract_comments)`；走 Chrome CDP 9222 复用登录态绕过小红书 300031 登录墙 |
| **Chrome CDP 启动方案**（aurora 登录态来源）| 用户启动 Chrome 时加 `--remote-debugging-port=9222 --user-data-dir=D:\chrome-cdp` | ✅ 在该 Chrome 里手动扫码登录小红书 → aurora 自动 detect localhost:9222 + reuse session；测试通过：完整拿到 markdown（标题 + 正文 + 视频转写 2136 字 + 评论 1428 条）|
| **xhs-pipeline plugin URL token 修复**（见 ADR-024）| `xhs-content-pipeline/__init__.py:120` | ✅ `_summarize_for_agent` 字段优先级改为 `raw_xhs_url` 优先；修复前 5/5 huitun URL 404 进硬数据模式，修复后 1/3 命中可解析（剩余仍 404 是笔记本身已删）|
...
| ADR-021 | **视频解析 tool 由用户自接，本项目不实现 xhs_video_downloader** | 2026-05-22 | 用户已有现成视频解析工具会接入；本项目 Phase 4.5 (xhs_video_downloader) 放弃，避免重复造轮子 + 规避研究小号反爬维护成本。Plugin 只负责采集（huitun_*）和 SKILL 调用，视频内容由用户外部喂入；保持 ADR-015 agent-first 原子能力组合。**ADR-022 落实**：用老板的 aurora plugin 满足这块能力 |
| ADR-022 | **Aurora plugin (wangedoo518/aurora) + Chrome CDP 9222 作为视频/图文解析方案** | 2026-05-23 | 老板独立 repo aurora 提供 `xhs_extract_note(url, ocr, transcribe, extract_comments)` —— 完整覆盖图文 + 视频 + 评论抓取，零外部 Python 依赖（用 Hermes 内置 STT + browser）。集成路径：(a) 本地 git clone aurora 到项目同级 (b) junction `aurora/plugins/xiaohongshu/` 到 `~/.hermes/plugins/xiaohongshu/` (c) 本地 patch aurora `__init__.py` 把 `from plugins.xiaohongshu.tools` 改成 `from .tools`（user plugin loader 不在 `plugins.xxx` 命名空间下，absolute import 找不到 module） (d) 用户启动 Chrome 时加 `--remote-debugging-port=9222 --user-data-dir=D:\chrome-cdp`，在该 Chrome 里登录小红书 → aurora 自动 detect CDP + 复用登录态。运行期约束：CDP Chrome 必须保持开 + 已登录；session token 跟着小红书自身 session 走。aurora 在 `.gitignore` 里屏蔽不入我们库 |
...
**Updated 2026-05-23**: Aurora plugin (老板 xhs_extract_note) 集成走通 + Chrome CDP 9222 复用登录态 (ADR-022); plugin URL token 字段优先级修复让 huitun 抓的 URL 能通过登录墙 (ADR-024); 端到端 CLI 链路 production-ready（2 分 15 秒跑完整 5 步，10 评分点完整，vol.14 路飞首稿可交付）; 三通道差异记录 (ADR-023): CLI ✅ / Weixin gateway ⚠️ iLink outbound rate limit / Dashboard chat ❌ 需 WSL2
**Updated 2026-05-23 (end of day)**: weixin platform_toolsets 缺 weixin 条目导致 agent 在微信上看不到 plugin tools，修法记在 ADR-025；**端到端 v4 在微信里 100% 跑通**：用户发 prompt → agent 调 huitun_search + xhs_extract_note + 3 SKILL → 5 报告落盘 + 微信送达 + agent 自主做"4 版迭代对比总览"。**路飞 vol.14 字节作品集深挖面试**逐字稿预期 CES 7,380（郑01 竞品的 11 倍）；7d 真空期 + 赛道无大 V 入场 = 路飞发布最佳时机。**项目 Phase 4 全部目标达成**
